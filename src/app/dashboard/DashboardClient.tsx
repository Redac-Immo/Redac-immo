  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1px', background: T.border }}>
        {[
          { label: 'Date de début', value: profile?.created_at ? new Date(profile.created_at).toLocaleDateString('fr-FR') : '—', sub: 'Activation initiale' },
          ...
          { label: 'Statut paiement', value: profile?.subscription_status === 'active' ? 'À jour' : 'Actif', sub: profile?.subscription_status === 'active' ? 'Carte enregistrée' : 'Paiement unique', ok: true },
        ].map(cell => (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
        <a href="/app" style={{ padding: '11px 24px', background: 'transparent', border: `1px solid ${T.border}`, color: T.mid, textDecoration: 'none', fontFamily: "'DM Sans', sans-serif", fontSize: '11px', letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500 }}>Aller à l'interface de génération →</a>
        <span style={{ fontSize: '11px', color: T.mid }}>Paiement sécurisé par Stripe</span>
      </div>
    </div>
  )
}
...
function SectionFactures() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1px', background: T.border }}>
        {[{ label: 'Total facturé 2026', value: '—', sub: 'Stripe bientôt disponible' }, { label: 'Abonnements réglés', value: '—', sub: 'En attente' }, { label: 'Annonces à l\'unité', value: '—', sub: 'En attente' }].map(kpi => (
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, padding: '48px', textAlign: 'center', color: T.mid }}><div style={{ fontSize: '24px', marginBottom: '12px' }}>🧾</div><div>Historique de facturation disponible après intégration Stripe.</div></div>
    </div>
  )
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '24px', alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <Card title="Formulaire de contact">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <Field label="Sujet"><select value={subject} onChange={e => setSubject(e.target.value)} style={inputStyle}><option>Problème technique</option><option>Question sur ma facture</option><option>Question sur mon abonnement</option><option>Suggestion d'amélioration</option><option>Autre</option></select></Field>
              <Field label="Message"><textarea value={message} onChange={e => setMessage(e.target.value)} rows={5} style={{ ...inputStyle, resize: 'vertical' as const }} placeholder="Décrivez votre question…" /></Field>
              <Btn onClick={handleSend} disabled={sending || !message.trim()}>{sending ? 'Envoi…' : 'Envoyer le message'}</Btn>
            </div>
          </Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {faqs.map((faq, i) => (
                <div key={i} style={{ borderBottom: i < faqs.length - 1 ? `1px solid ${T.border}` : 'none' }}>
                  <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ padding: '16px 0', fontSize: '13px', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'none', border: 'none', width: '100%', textAlign: 'left', fontFamily: "'DM Sans', sans-serif", color: T.dark, transition: 'color 0.15s' }}>
                    {faq.q}
                  ...
                  {openFaq === i && <div style={{ paddingBottom: '16px', fontSize: '13px', color: T.mid, lineHeight: 1.7 }}>{faq.a}</div>}
                </div>
              ))}
            </div>
          </Card>
        ...
        <div style={{ background: T.bg, padding: '28px', display: 'flex', flexDirection: 'column', gap: '16px', position: 'sticky', top: '80px' }}>
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '20px', fontWeight: 300 }}>Besoin d'une <em style={{ fontStyle: 'italic', color: T.gold }}>réponse rapide</em> ?</div>
          <div style={{ fontSize: '12px', color: '#888', lineHeight: 1.6 }}>Notre équipe est disponible pour vous accompagner sur toute question technique, commerciale ou relative à votre compte.</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <button onClick={() => showToast('Redirection vers la messagerie…')} style={{ padding: '11px 24px', background: T.gold, color: T.bg, border: 'none', fontFamily: "'DM Sans', sans-serif", fontSize: '11px', letterSpacing: '0.16em', textTransform: 'uppercase', cursor: 'pointer', fontWeight: 500, width: '100%' }}>Envoyer un e-mail</button>
            <button onClick={() => showToast('FAQ complète — bientôt disponible')} style={{ padding: '11px 24px', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: '#AAA', fontFamily: "'DM Sans', sans-serif", fontSize: '11px', letterSpacing: '0.16em', textTransform: 'uppercase', cursor: 'pointer', width: '100%' }}>Voir la FAQ complète →</button>
          </div>
          <div style={{ fontSize: '11px', color: T.mid, padding: '12px 0', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ marginBottom: '4px', color: '#666', fontSize: '10px', letterSpacing: '0.14em', textTransform: 'uppercase' }}>Horaires</div>Lun — Ven · 9h → 18h (CET)<br /><span style={{ color: '#555' }}>Réponse garantie sous 24h ouvrées</span>
          </div>
        </div>
      </div>
    </div>
...
function StatusBadge({ statut }: { statut: string }) {
  const config = { encours: { type: 'gold' as const, label: 'En cours' }, vendu: { type: 'ok' as const, label: 'Vendu' }, archive: { type: 'mid' as const, label: 'Archivé' } }[statut] ?? { type: 'mid' as const, label: statut }
